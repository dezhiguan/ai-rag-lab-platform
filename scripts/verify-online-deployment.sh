...
status_code=$(http_code "$PUBLIC_BASE_URL/api/system/status")
if [[ "$status_code" == "200" ]]; then
  pass_msg "System status API: HTTP $status_code"
  status_body=$(curl -sf --connect-timeout 15 "$PUBLIC_BASE_URL/api/system/status" 2>/dev/null || true)
...
# Sample status
sample_code=$(http_code "$PUBLIC_BASE_URL/api/sample/status")
